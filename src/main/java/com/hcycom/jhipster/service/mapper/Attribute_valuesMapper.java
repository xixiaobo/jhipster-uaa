...
public interface Attribute_valuesMapper {
	...
	@Select("SELECT COUNT(a.uuid) FROM (SELECT uuid FROM `${attribute_values.save_table}` GROUP BY uuid) a")
	public int findAttribute_valuesCountByID(@Param("attribute_values") Attribute_values attribute_values);
